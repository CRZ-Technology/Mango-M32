	...
	uint32_t byteswritten;
 	...
 	char wtext[100] = "This line has been written by an STM32 device!\n";
  	if(f_mount(&USBHFatFS, (TCHAR const*) USBHPath, 0) == FR_OK)
  	{
  		...
  		if (res != FR_OK) {	printf("f_open() fail, res=%d\r\n", res);  return; }
  		printf("f_open() ok\r\n");

  		res = f_write(&USBHFile, wtext, strlen(wtext), (void *)&byteswritten);
  		if (res != FR_OK) {	printf("f_write() fail, res=%d\r\n", res);  return; }
  		printf("f_write() ok\r\n");

  		res = f_sync(&USBHFile);
  		if (res != FR_OK) {	printf("f_sync() fail, res=%d\r\n", res);  return; }
  		printf("f_sync() ok\r\n");

  		res = f_close(&USBHFile);
  		if (res != FR_OK) {	printf("f_close() fail, res=%d\r\n", res);  return; }
  		printf("f_close() ok\r\n");
  	}
  }
  break;
